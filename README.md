...
**16 аналитических эндпоинтов**, вся логика — в одном файле [`result-service/app/analytics.py`](services/result-service/app/analytics.py).

### Цифры на демо-наборе

> 77 учеников, 6 недельных когорт, 82 попытки. Данные синтетические, но реалистично распределённые — генератор [`scripts/seed_demo_data.py`](scripts/seed_demo_data.py) моделирует 5 поведенческих групп: возвращающихся, угасающих, одноразовых, не начавших и бросивших тест.
...
- Статистическая интерпретация A/B-теста (p-value, доверительный интервал) через ExperimentHub
- Серверная валидация ответов вместо клиентского `is_correct`

**Сознательно вне скоупа:** LTV и монетизационные метрики — у продукта нет денежной модели, такие цифры были бы искусственными. Recommendation Engine и Search Ranking — это отдельная ML-задача, а не продуктовая аналитика.

---

## Связанные проекты

**[ExperimentHub](https://github.com/kuznetsovskdh/experimenthub)** — внешняя платформа A/B-тестирования. РусТест обращается к ней за назначением варианта и отправляет метрики; рандомизация и статистика живут в отдельном сервисе, а не внутри продукта. Оба вызова с таймаутом 2 секунды и подавлением ошибок — недоступность платформы экспериментов не мешает проходить тесты.

---

## Автор

Илья Кузнецов — [GitHub](https://github.com/kuznetsovskdh)
