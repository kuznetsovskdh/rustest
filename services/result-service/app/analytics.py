...
def get_progress(user_id: int, db: Session):
    ...
    for a in attempts:
        ...
        rows.append({
            "date": a.finished_at.strftime("%Y-%m-%d"),
            "test_id": a.test_id,
            "score_pct": pct
        })
    ...
    for e in errors:
        if e["error_rate"] >= 60:
            recs.append({
                ...
                "message": f"Вопрос #{e['question_id']}: высокий процент ошибок ({e['error_rate']}%) — повторите тему"
            })
        elif e["error_rate"] >= 30:
            recs.append({
                "question_id": e["question_id"],
                "message": f"Вопрос #{e['question_id']}: есть ошибки ({e['error_rate']}%) — стоит потренироваться"
            })
    return recs
